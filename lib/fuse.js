...
module.exports = class Fuse {
  constructor (fs, mountDir) {
    this.fs = fs
    this.mountDir = mountDir || './hdp'
    this.mounted = false
  }
  ...
  async mount () {
    ...
    this.fuse = this.fuse || new FuseNative(this.mountDir, this.handlers, { autoCache: false })
    this.mounted = true
    ...
    return {
      getattr (p, cb) {
        ...
        fs.stat(p)
          .then((stat) => { cb(0, stat) })
          .catch((err) => {
            if (err.errno === Fuse.EIO) console.log('********', err)
