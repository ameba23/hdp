...
function defineClientMessage () {
  ...
  function encodingLength (obj) {
    var length = 0
    if ((+defined(obj.readdir) + +defined(obj.stat) + +defined(obj.open) + +defined(obj.read) + +defined(obj.close)) > 1) throw new Error("only one of the properties defined in oneof msg can be set")
  ...
  function defineReaddir () {
    ...
    function encodingLength (obj) {
      ...
      if (defined(obj.files)) {
        for (var i = 0; i < obj.files.length; i++) {
          ...
          var len = encodings.string.encodingLength(obj.files[i])
          length += 1 + len
        }
      }
      return length
    ...
    function encode (obj, buf, offset) {
      ...
      if (defined(obj.files)) {
        for (var i = 0; i < obj.files.length; i++) {
          if (!defined(obj.files[i])) continue
          buf[offset++] = 10
          encodings.string.encode(obj.files[i], buf, offset)
          offset += encodings.string.encode.bytes
        }
      }
      encode.bytes = offset - oldOffset
    ...
    function decode (buf, offset, end) {
      ...
      while (true) {
        ...
        switch (tag) {
          case 1:
          obj.files.push(encodings.string.decode(buf, offset))
          offset += encodings.string.decode.bytes
          break
          default:
          offset = skip(prefix & 7, buf, offset)
