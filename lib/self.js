...
module.exports = class Self {
  ...
  rootStat () {
    return {
      atime: this.rpc.ctime, // TODO
      mtime: this.rpc.ctime,
      ctime: this.rpc.ctime,
      size: 4096
    }
  }
  ...
  async find (basepath, searchterm) {
    ...
    // for await (const response of this._multiRpcCall({ find: { basepath, searchterm } })) {
    //   if (response.err) throw createError(response.err)
    //   yield response.success.find.results
    // }
}
