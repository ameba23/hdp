...
module.exports = function (hdp, server) {
  ...
  wsServer.on('connection', async (connection) => {
    ...
    hdp.on('peerConnected', (name) => {
      connection.send(newMessage({ peerConnected: { name } }))
    })

    hdp.on('peerDisconnected', (name) => {
      connection.send(newMessage({ peerDisconnected: { name } }))
    })
    ...
    const downloadedFiles = await hdp.fs.getDownloadedList()
    connection.send(newMessage({
