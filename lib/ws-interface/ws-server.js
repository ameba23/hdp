...
module.exports = function (hdp, server) {
  ...
  const commands = {
    ...
    async * swarm ({ name, join }) {
      ...
      await hdp[joinOrLeave](name)

      yield {
        connected: Object.entries(hdp.swarms).filter(s => s[1]).map(s => s[0]),
        disconnected: Object.entries(hdp.swarms).filter(s => !s[1]).map(s => s[0])
      }
    }
  }
  ...
  wsServer.on('connection', (connection) => {
    ...
    connection.on('message', async (data) => {
      ...
      if (typeof commands[command] === 'function') {
        ...
        await handleCommand(iterator, command, message.id)
      }
    })

    // Handle internal events
    hdp.fs.on('internalCommand', async (iterator, messageType) => {
      // TODO give a random id
      // message.id = ...
      const id = randomBytes(4).readInt32BE()
      await handleCommand(iterator, messageType, id)
    })
    ...
    async function handleCommand (iterator, messageType, id) {
      try {
        for await (const output of iterator) {
          log('Writing output', messageType, output)
        }
