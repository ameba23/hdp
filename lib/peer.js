...
module.exports = class Peer extends EventEmitter {
  constructor (connection, hdp) {
    ...
    connection.on('data', async (data) => {
      ...
      switch (Object.keys(message)[0]) {
        case 'readDirRequest':
          try {
            const output = await hdp.onReadDir(message.readDirRequest.path, self.remotePk)
        ...
        case 'openRequest':
          try {
            const fd = await hdp.onOpen(message.openRequest.path, self.remotePk)
        ...
        case 'readRequest':
          try {
            ...
            const { data, bytesRead } = await hdp.onRead(fd, len, pos, self.remotePk)
  ...
  // Generic version of the methods below
  sendResponse(messageType, err, output, path) {
    const responseMessage = {}
    responseMessage[messageType] = err ? { err: err.errno } : output
  ...
  respondOpen (err, fd, path) {
    const openResponse = err ? { err: err.errno } : { fd }
    openResponse.path = path
    this.connection.write(HdpMessage.encode({ openResponse }))
  }
}
