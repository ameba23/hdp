    ...
    connection.on('data', (data) => {
      ...
      switch (Object.keys(message)[0]) {
        ...
        case 'statRequest':
          ...
          break
        case 'openRequest':
          hdp.onOpen(message.openRequest.path, self.remotePk)
          break
        case 'readRequest':
          const { fd, len, pos } = message.readRequest
          hdp.onRead(fd, len, pos, self.remotePk)
          break
        case 'closeRequest':
          hdp.onClose(message.closeRequest.fd, self.remotePk)
          break
        ...
        case 'statResponse':
          ...
          break
        case 'openResponse':
          self.emit('openResponse', message.openResponse)
          break
        case 'readResponse':
          self.emit('readResponse', message.readResponse)
          break
        case 'closeResponse':
          self.emit('closeResponse', message.closeResponse)
          break
      }
    })
