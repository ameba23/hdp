...
const { ECONNABORTED } = require('fuse-native')
...
module.exports = class Peer extends EventEmitter {
  ...
  setConnection (connection) {
    ...
    connection.on('data', async (data) => {
      ...
      let output
      try {
        output = await self.rpc.handleRequest(request)
      } catch (err) {
        console.log(err)
    ...
    //     self.emit(requestId, { err: ECONNABORTED })
    //   }
    // })
  }
  ...
  async readdir (path) {
    ...
    if (!response) {
      ...
      // TODO if this is an error response, make sure we want to cache it
  ...
  async _rpcCall (request) {
    const self = this
    if (this.connection.destroyed) throw createError(ECONNABORTED)
