...
module.exports = class Fs {
  ...
  // readdir here means names only
  async readdir (pathString) {
    const files = await this.ls(pathString)
  ...
  async stat (pathString) {
    ...
    if (this._connectedPeerNames().includes(path[0])) {
      ...
      const containingDir = await this.ls(path.slice(0, -1).join('/'))
      const stat = containingDir.find(s => s.name === path.slice(-1)[0])
      if (stat) return stat
    }
    throw createError(ENOENT)
  }

  async * createReadStream (pathString) {
